...
const NO_VALUE = {};
const idx2D = (row, col) => row * SETTINGS.WIDTH + col;
const block = (f) => f();
const table = document.querySelector("#table-container")
    .appendChild(document.createElement("table"));
const changeCellState = (cell, from, to, value, imgUrl) => {
    if (!from.delete(cell))
        return false;
    to.set(cell, value);
...
const game = block(() => {
    ...
    const cells = Array.from({ length }, (_, i) => {
        ...
        button.addEventListener("click", (event) => {
            const i = event.target.id.charCodeAt(0);
            const cell = newGame.cells[i];
            changeCellState(cell, game.cellState.poster, game.cellState.empty, NO_VALUE, IMAGES.NONE);
        });
...
const gameLoop = () => {
    ...
    [...game.cellState.poster]
        .filter((_, v) => v <= 0.0)
        .forEach(c => {
        changeCellState(c[0], game.cellState.poster, game.cellState.building, NO_VALUE, IMAGES.NONE);
    });
};
